...
public class CategoryEng {
    ...
    public CategoryEng(String source) {
        this.source = source;
    ...
    private void init() {
        String sql = "select code, keywords from category_engine";
        try {
...
//                    new_doc.add(new Field("jointime", doc.get("jointime"), Field.Store.YES, Field.Index.UN_TOKENIZED));
                    ...
                    doc.add(new Field("cate_eng", c, Field.Store.YES, Field.Index.TOKENIZED));
                    iw.addDocument(doc);
                }
            }
            reader.close();
